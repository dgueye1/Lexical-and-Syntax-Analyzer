...
bool KeyWord(char* s)
{
    if (!strcmp(s, "if") || !strcmp(s, "else") ||
        ...
        || !strcmp(s, "struct") || !strcmp(s, "goto") || !strcmp(s, "for"))
        return (true);
    return (false);
}
...
char assignments(char *nextToken) {
  ...
  if (strstr(nextToken, "+=") || strstr(nextToken,"-=")||strstr(nextToken, "*=")||strstr(nextToken, "/=")||strstr(nextToken, "%=")||strstr(nextToken, "==")||strstr(nextToken, "!=")||strstr(nextToken, ">") || strstr(nextToken, "<") || strstr(nextToken, ">=") || strstr(nextToken, "<=") || strstr(nextToken, "&&") || strstr(nextToken, "||")|| strstr(nextToken, "!")){
    printf("passes syntax analyzer\n");
    check(nextToken);
  }
 else{
...
int main()
{
     //Write what you would like to check here
   returnstmt("if(i < 5) return 2;");
